...
import css from './AddToGardenBtn.module.css';

export default function AddToGardenBtn({ speciesId }: { speciesId: string }) {
  const isAuthenticated = useAuthStore(state => state.isAuthenticated);

  const handleClick = () => {
    if (isAuthenticated) {
      console.log('open add plant modal', speciesId);
      return;
    }
    console.log('open attention modal');
  };
